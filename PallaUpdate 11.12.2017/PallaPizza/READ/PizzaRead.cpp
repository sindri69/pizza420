...
double PizzaRead::getTotalPrice(string pID) { /// Here we calculate the total price for all the pizzas matching the pID
    UserInputUI ui;
    int linecount = 1;
    double totalprice = 0;
    for (int i = 0; i < (get_NumberOfLines() / NUMBEROFVALUESINPIZZA); i++) {
        if (ui.convert_StringToLowerCase(get_Line(linecount)) == ui.convert_StringToLowerCase(pID)) {
            totalprice = totalprice + ui.convert_StringToDouble(get_Line(linecount + 4));
        }
