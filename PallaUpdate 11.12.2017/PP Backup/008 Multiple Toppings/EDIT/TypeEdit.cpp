...
void TypeEdit::deleteType() { /// Here we open and close the type file without ios::app, this removes all lines in the file
    ofstream fout;
    fout.open ("type.txt");
    fout.close();
}
void TypeEdit::deleteType(int typeNUM) { /// Here we delete a type which matches the number
    TypeRead tr;
    if (tr.fileNotEmpty()) {
...
void TypeEdit::deleteType(string typeNAME) { /// Here we delete a type which matches the name
    ...
    for (int i = 0; i < (tr.get_NumberOfLines() / NUMBEROFVALUESINTYPE); i++) { /// We need to find the typeNUM that matches this typeNAME
        if (ui.convert_StringToLowerCase(tr.get_Line(linecount)) == ui.convert_StringToLowerCase(typeNAME)) {
            namefound = true;
            break;
        }
