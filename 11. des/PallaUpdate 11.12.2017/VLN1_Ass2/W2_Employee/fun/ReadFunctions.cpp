...
#include <sstream>
#include <string>
#include <algorithm> /// Used for finding element in vector
using namespace std;
...
void ReadFunctions::read_AllSalaryRecords_employee(string ssn) {
    int linecount = 1;
    if (fileNotEmpty_employee()) {
        for (int i = 0; i < (get_NumberOfLines_employee() / NUMBEROFVALUESINEMPLOYEE); i++) {
            ...
            if (check_LineMatches_employee(ssn,linecount)) {
                string name = get_Line_employee(linecount - 1);
                string salary = get_Line_employee(linecount + 1);
                string month = get_Line_employee(linecount + 2);
                string year = get_Line_employee(linecount + 3);
...
void ReadFunctions::get_TotalSalary(string ssn, string year) {
    MyFunctions myfun;
    ...
    if (fileNotEmpty_employee()) {
        for (int i = 0; i < (get_NumberOfLines_employee() / NUMBEROFVALUESINEMPLOYEE); i++) {
            ...
            if (check_LineMatches_employee(year,linecount)) {
                if (namematches) {
                    totalsalary = totalsalary + myfun.convert_StringToDouble(get_Line_employee(linecount - 2));
                }
            }
            linecount++;
            namematches = false;
        }
        cout << "SSN: " << ssn << " total salary for the year " << year << " = " << totalsalary << endl;
    }
    else {
        cout << "File is empty" << endl;
    }
}
double ReadFunctions::get_TotalSalary_ReturnSalary(string ssn, string year) {
    MyFunctions myfun;
    bool namematches = false;
    double totalsalary = 0;
    int linecount = 1;
...
void ReadFunctions::find_HighestEarner_employee(string year) {
    vector <string> uniqueSSN; /// First we gather all the unique SSN into an array
    int linecount = 1;
    for (int i = 0; i < (get_NumberOfLines_employee() / NUMBEROFVALUESINEMPLOYEE); i++) { /// For every individual employee record ...
        if (get_Line_employee(linecount + 4) == year) { /// If this record is for the year we're looking for
            if (!( find(uniqueSSN.begin(),uniqueSSN.end(),get_Line_employee(linecount + 1)) != uniqueSSN.end() )) { /// If this is a new SSN, add it to the array
                uniqueSSN.push_back(get_Line_employee(linecount + 1));
            }
        }
        linecount = linecount + 5;
    }
    string ssnofhighestsalary;
    double highestsalary = 0;
    double newsalary = 0;
    for (unsigned int i = 0; i < uniqueSSN.size(); i++) {
        newsalary = get_TotalSalary_ReturnSalary(uniqueSSN[i],year);
        if (newsalary > highestsalary) {
            highestsalary = newsalary;
            ssnofhighestsalary = uniqueSSN[i];
        }
    }
    cout << "SSN: " << ssnofhighestsalary << " has the highest salary (" << highestsalary << ") for the year " << year << endl;
}
