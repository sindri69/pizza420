        ...
        while (!lasttopping) {
            ...
            if (ui.answerIsInteger(answer)) {
                ...
                if ( toppingnum <= 0 || toppingnum > (tor.get_NumberOfLines() / NUMBEROFVALUESINTOPPING) ) { /// If the answer is a number, we try to match that number to the ones on the list
                    ...
                    tVector.push_back(tor.get_Line( (toppingnum * NUMBEROFVALUESINTOPPING) - NUMBEROFVALUESINTOPPING + 1 )); /// If we want topping 1 we take line 1, If we want topping 2  we take line 5 ... etc
                    cout << endl;
                }
            }
            else {
                if (tor.check_ToppingAlreadyExists(answer)) { /// If the answer is not a number we try to match it to the names on the list
                    tVector.push_back(answer);
                }
                else {
                    cout << "That topping is not on the list" << endl << endl;
                }
            }
            if (!ui.getAnswer_Yes_Or_No("Would you like to add another topping?")) {
                lasttopping = true;
            }
        }
        string alltopping = tVector[0];
        if (tVector.size() > 1) {
            for (unsigned int i = 1; i < tVector.size(); i++) {
                alltopping = alltopping + "\\" + tVector[i];
            }
        }
        return alltopping;
    }
...
double GetCorrectPizzaUI::getCorrectPrice(string pID, string psize, string ptype, string ptopping) {
    BasicRead br;
    TypeRead tyr;
    ToppingRead tor;
    UserInputUI ui;
    ...
    if (ptype == "Basic") { /// If the pizza is a Basic-type then we get the price from the basic text file
        ...
        for (unsigned int i = 0; i < t.size(); i++) { /// for each topping in the vector ...
            legitprice = legitprice + getCorrectPriceTopping(t[i],psize);
        }
        return legitprice;
    }
    ...
    legitprice = ui.convert_StringToDouble(price);

    return legitprice;
}
double GetCorrectPizzaUI::getCorrectPriceTopping(string tname, string psize) {
    ToppingRead tor;
    UserInputUI ui;
    ...
    for (int i = 0; i < tor.get_NumberOfLines(); i++) { /// We loop out way to the line where the topping is
        linecount ++;
        if (tor.check_LineMatches(tname,linecount)) {
            break;
        }
    }
    if (psize == "Large") {
    ...
    return ui.convert_StringToDouble(price);
}
