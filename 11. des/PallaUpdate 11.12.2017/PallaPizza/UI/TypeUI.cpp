...
void TypeUI::start_TypesMenu() {
    ...
    while (input != 'q') {
        ...
        if (input == '1') { /// Here we make new PizzaTypes, each Type must contain at least one character, and it can not have the same name as an existing type
            ...
            catch(InvalidTypeNameBasicIsReservedException e) {
                cout << "The Type-name \"Basic\" is reserved" << endl << endl;
            }
            catch(InvalidTypeNameCustomIsReservedException e) {
                cout << "The Type-name \"Custom\" is reserved" << endl << endl;
            }
        ...
        else if (input == '3') { /// Here we remove a type from the types-list
            if (tr.fileNotEmpty()) { /// If the list is empty, there's nothing there to edit ...
                cout << "Types on the Types-List" << endl;
                tv.viewType(); /// First we display the list of types that the user can edit
                cout << endl;
                ...
                if (ui.answerIsInteger(whichtype)) { /// If answer is an integer, we take the type that matches that number on the list
                    ...
                    if ( whichnum <= 0 || whichnum > (tr.get_NumberOfLines() / NUMBEROFVALUESINTYPE) ) {
                        cout << "That number does not match the ones on the list" << endl << endl;
                    }
                    else {
                        te.deleteType(whichnum);
                        cout << endl;
                    }
                }
                else {
                    if (tr.check_TypeAlreadyExists(whichtype)) { /// If answer is not an integer we try to match the user input to the names on the list
                        te.deleteType(whichtype);
                        cout << endl;
                    }
                    else {
